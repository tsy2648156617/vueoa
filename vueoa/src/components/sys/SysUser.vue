	...
	<el-dialog title="编辑" v-model="editVisible" width="30%">
		<el-form ref="form" :model="form" label-width="70px">
			...
			<el-form-item label="部门">
				<el-select v-model="selectTreeName" placeholder="请选择">
					<el-option :value="selectTreeName" :label="deptTitle" class="options">
						<el-tree id="tree-option" ref="selectTree" :accordion="true" :data="depts" :props="props" :default-expand-all="true" :expand-on-click-node="false" node-key="deptId" :default-expanded-keys="[]" @node-click="handleNodeClick">
						</el-tree>
					</el-option>
				</el-select>
			</el-form-item>
			...
			<el-form-item label="职务">
				<el-select v-model="form.role" placeholder="请选择">
					<el-option v-for="item in roles" :key="item.roleId" :label="item.roleName" :value="item">
					</el-option>
				</el-select>
			</el-form-item>
			<el-form-item label="审批列表">
				<el-select v-model="form.approvedIds" multiple placeholder="请选择">
					<el-option v-for="item in tableData" :key="item.uid" :label="item.uname" :value="item.uid">
					</el-option>
				</el-select>
			</el-form-item>
		...
		<template #footer>
			<span class="dialog-footer">
				...
				<el-button type="primary" @click="reqWay == 'post' ? saveAddUser() : saveEdit()">确 定</el-button>
			</span>
		</template>
	</el-dialog>
</template>
